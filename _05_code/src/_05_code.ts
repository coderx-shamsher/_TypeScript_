...
function testing(kind: string | number){
...
function testingf1(mesg?:string){
    if(mesg){
        return `${mesg} hello !`
    }
...
// exzustive checking 
function username(user: "admin" | "admin0x" | number){
...
// build our own types using the type keyword
// hamne primary data types ka use kiya hai type create krne k liye 
type type_user = {
    type : string 
    username : string
    role :  string
